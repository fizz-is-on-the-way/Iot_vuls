...
TOTOLINK N150RT V2_Firmware V3.4.0-B20190525 contains a buffer overflow vulnerability in /boafrm/formStaticDHCP  `hostname` parameter.
...
We can see that the `v11` variable receives the `hostname` parameter from a POST request. However, since the user can control the input of `hostname` parameter, the statement `strcpy(&v33[13], v11);` can cause a buffer overflow vulnerability.
