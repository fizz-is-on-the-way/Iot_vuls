...
TOTOLINK N150RT V2_Firmware V3.4.0-B20190525 contains a buffer overflow vulnerability in /boafrm/formWsc  `submit-url` parameter.
...
We can see that the `v4` variable receives the `submit-url` parameter from a POST request. However, since the user can control the input of `submit-url` parameter, the statement `strcpy(&byte_477DA0, v4);` can cause a buffer overflow vulnerability.
